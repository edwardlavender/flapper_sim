...
# ... when implemented 'correctly' by comparing KUD maps for the simulated path
# ... to those reconstructed by the COA & flapper family of algorithms
# ... based on a {spatstat} smoothing method
...
# 3) This code is closely modelled on sim_evaluate.R but
# ... uses a different KUD method.
...
estimates_by_array_best_file <-
  paste0("./data/estimates/path_", path_id, "/estimates_by_array_best_spatstat.rds")
if(TRUE | !file.exists(estimates_by_array_best_file)){
  ...
  estimates_by_array <- lapply(1:length(dat_sim_arrays), function(array_id){
    ...
    path_kud <- pf_dens(grid,
                        coord = path$xy_mat_on_grid_within_acoustics,
                        sigma = sigma,
                        plot = FALSE)
    ...
    #### KUD for COAs
    coa_kud <- pf_dens(grid,
                       coord = out_coa,
                       sigma = sigma,
                       plot = FALSE)

    #### POU for ACPF and ACDCPF
    ...
    #### KUD for ACPF and ACDCPF
    acpf_kud <- pf_dens(acpf_pou,
                        sigma = sigma,
                        plot = FALSE)
    acdcpf_kud <- pf_dens(acdcpf_pou,
                          sigma = sigma,
                          plot = FALSE)
...
#### Set up image
save_png <- TRUE
if(save_png) png(paste0("./fig/evaluation/path_", path_id, "_array_evaluation_", type, "_", sbt, "_spatstat.png"),
