...
1. `R/` contains scripts for data simulation and analysis. 
    ...
    * `sim_evaluate.R` evaluates the performance of the `flapper` family of algorithms, when applied 'correctly', in different array designs and `sim_evaluate_wall_time.R` calculates the wall time for these simulations. 
...
4. Evaluate algorithm outputs when applied 'correctly' via `sim_illustrate.R`, `sim_evaluate.R` and `sim_evaluate_wall_time.R`. 
