...
2. Implement algorithms for a specific movement path and array combination via `sim_workhorse.R` or for all movement paths and array designs via `sim_implement.R`*. Note that for the former, the movement path and array ID need to be specified at the top of the script.

3. Examine and synthesise outputs via `sim_illustrate.R`, `sim_synthesise.R` and `sim_wall_time.R`. 

4. Examine outputs in `data/` and `fig/`, such as the example figure below. 
